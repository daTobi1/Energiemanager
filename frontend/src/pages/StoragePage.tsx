...
import { Plus, Trash2, Edit2, Battery, X, Copy, Thermometer, Snowflake } from 'lucide-react'
...
const typeColors: Record<StorageType, string> = {
  battery: 'bg-purple-100 text-purple-700',
          ...
          {isThermal && (
            <>
              ...
              <Section title="Temperatursensoren" defaultOpen={true} badge={`${(editing as ThermalStorage).temperatureSensors.length} Sensoren`}>
                <p className="text-sm text-gray-500 mb-3">Temperatursensoren für die Speicherüberwachung und selbstlernende Regelung</p>
                {(editing as ThermalStorage).temperatureSensors.map((sensor, i) => (
                  <div key={sensor.id} className="p-3 bg-gray-50 rounded-lg mb-3">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-semibold text-gray-700">Sensor {i + 1}</span>
                      <button onClick={() => {
              ...
              <Section title="Anschlüsse / Zuordnungen" defaultOpen={true}>
                <p className="text-sm text-gray-500 mb-3">Welche Erzeuger und Verbraucher sind mit diesem Speicher verbunden?</p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {generatorOptions.length === 0 && <p className="text-xs text-gray-400">Noch keine Erzeuger angelegt</p>}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {consumerOptions.length === 0 && <p className="text-xs text-gray-400">Noch keine Verbraucher angelegt</p>}
                    </div>
                  </div>
                </div>
              </Section>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Batterie-, Wärme- und Kältespeicher konfigurieren</p>
        </div>
      </div>
        ...
        {storageTypeOptions.map(({ value, label }) => {
          ...
          return (
            <button key={value} onClick={() => startAdd(value as StorageType)}
              className="card hover:border-emerald-300 hover:shadow-md transition-all flex flex-col items-center gap-2 py-6 cursor-pointer">
      ...
      {storages.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-500">Noch keine Speicher konfiguriert</p>
        </div>
      ) : (
              ...
              <div key={s.id} className="card flex items-center gap-4 hover:shadow-md transition-shadow">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-gray-900">{s.name || 'Unbenannt'}</h3>
                    <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">{typeLabels[s.type]}</span>
                  </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => removeStorage(s.id)} className="btn-icon text-red-400 hover:text-red-600 hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            )
          })}
